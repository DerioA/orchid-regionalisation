...
#Merge the kingdoms into the sf file to continue with the analysis
#We already have an sf file with the kingdoms and bioregions. Let's load that sf file and do it
shape200 <- st_read("results/SIG/bioregion200km_realms-phyloregions.gpkg")
# Asegurarse de que los ID coincidan como character
shape200_sf <- shape200_sf %>% mutate(idcell = as.character(idcell))
shape200 <- shape200 %>% mutate(idcell = as.character(idcell))
# Seleccionar solo columnas necesarias de shape200
...
################################################################################
# ASSIGN MAJOR BIOGEOGRAPHIC LANDMASSES
# (TECTONICALLY INFORMED, ROBUST, REPRODUCIBLE)
################################################################################
...
# -------------------------------------------------------------------
# 1. Load world countries (used as tectonic/biogeographic proxies)
# -------------------------------------------------------------------
countries_sf <- ne_countries(
  scale = "medium",
  returnclass = "sf") %>%
  st_transform(st_crs(shape200_sf))

# -------------------------------------------------------------------
# 2. Assign CONTINENT (modern, only as auxiliary info)
# -------------------------------------------------------------------
...
# Sanity check
if (!"continent" %in% names(shape200_sf)) {
  stop("❌ Continent column was not created – spatial join failed.")
}

cat("\nContinent assignment:\n")
print(table(shape200_sf$continent, useNA = "ifany"))

# -------------------------------------------------------------------
# 3. Define countries belonging to the INDIAN SUBCONTINENT
#    (proxy for the Indo-Indian Plate)
# -------------------------------------------------------------------
india_countries <- c(
  "India",
  "Sri Lanka",
...
india_sf <- countries_sf %>%
  filter(admin %in% india_countries)

# -------------------------------------------------------------------
# 4. Create MAJOR BIOGEOGRAPHIC LANDMASSES
# -------------------------------------------------------------------
shape200_sf$landmass <- NA_character_

# ---- 4.1 Indian subcontinent (tectonically distinct) ----
india_hits <- st_intersects(shape200_sf, india_sf, sparse = FALSE)
shape200_sf$landmass[
  rowSums(india_hits) > 0
] <- "India"

# ---- 4.2 Africa ----
shape200_sf$landmass[
  is.na(shape200_sf$landmass) &
    shape200_sf$continent == "Africa"
] <- "Africa"

# ---- 4.3 Americas ----
shape200_sf$landmass[
  is.na(shape200_sf$landmass) &
    shape200_sf$continent == "North America"
] <- "NorthAmerica"

shape200_sf$landmass[
  is.na(shape200_sf$landmass) &
    shape200_sf$continent == "South America"
] <- "SouthAmerica"

# ---- 4.4 Australia ----
shape200_sf$landmass[
  is.na(shape200_sf$landmass) &
    shape200_sf$continent == "Oceania"
] <- "Australia"

# ---- 4.5 Eurasia (Europe + Asia EXCLUDING India) ----
shape200_sf$landmass[
  is.na(shape200_sf$landmass) &
    shape200_sf$continent %in% c("Europe", "Asia")
] <- "Eurasia"

# -------------------------------------------------------------------
# 5. Final sanity checks
# -------------------------------------------------------------------
cat("\nFinal landmass assignment:\n")
print(table(shape200_sf$landmass, useNA = "ifany"))

if (any(is.na(shape200_sf$landmass))) {
  warning("⚠ Some grid cells still have NA landmass – check oceanic cells.")
}

# -------------------------------------------------------------------
# REMOVE DUPLICATES AFTER SPATIAL JOIN (CRITICAL STEP)
# -------------------------------------------------------------------
shape200_sf <- shape200_sf %>%
  group_by(idcell) %>%
  slice(1) %>%     # keep first intersection only
  ungroup()

# -------------------------------------------------------------------
# 6. OPTIONAL: quick visual check (highly recommended)
# -------------------------------------------------------------------
ggplot(shape200_sf) +
  geom_sf(aes(fill = landmass), color = NA) +
  scale_fill_viridis_d() +
  theme_minimal() +
  labs(
    title = "Major biogeographic landmasses (tectonic proxy)",
    fill = "Landmass")
table(shape200_sf$landmass, useNA = "ifany")

shape200_sf <- shape200_sf %>%
  filter(!is.na(landmass))
...
env_data <- shape200_sf %>% 
  dplyr::select(idcell, realm, landmass, all_of(vars)) %>%
  filter(if_all(all_of(vars), ~ !is.na(.))) %>%
  mutate(across(all_of(vars), scale)) %>%
...
###############################################################################
# PAIRWISE CONTINENTAL ISOLATION DISTANCE
###############################################################################

pairs_df <- pairs_df %>%
  mutate(
    dist_landmass_raw = ifelse(landmass.i == landmass.j, 0, 1),
    dist_landmass = scale(dist_landmass_raw))

table(pairs_df$dist_landmass_raw)
###############################################################################
...
correspondencia <- tibble::tibble(
  realm = factor(1:6),
  region_name = c("Holartic","Australian","Andean",
                  "Neotropical","Afrotropical","Indo-Malaysian"))
# Assign names by join, not by factor directly
pairs_df <- pairs_df %>%
  left_join(correspondencia, by = c("phyloregion_i" = "realm")) %>%
...
# 1. Australian vs all other groups
regA1 <- "Australian"
regB1 <- c("Andean", "Neotropical", "Afrotropical", "Holartic", "Indo-Malaysian")
# 2. Andean vs Neotropical y Afrotropical
regA2 <- "Andean"
regB2 <- c("Neotropical", "Afrotropical")
# 3. Neotropical vs Afrotropical
regA3 <- "Neotropical"
...
context1_intraB <- pairs_df %>% 
  filter(phyloregion_i_name == phyloregion_j_name,
         phyloregion_i_name %in% c("Andean", "Neotropical", "Afrotropical", "Holartic", "Indo-Malaysian"))
...
cat("\n\n--- Between A and B (Australian vs Others) ---\n")
context1_inter <- pairs_df %>% 
  filter((phyloregion_i_name == "Australian" & phyloregion_j_name %in% c("Andean", "Neotropical", "Afrotropical", "Holartic", "Indo-Malaysian")) |
           (phyloregion_j_name == "Australian" & phyloregion_i_name %in% c("Andean", "Neotropical", "Afrotropical", "Holartic", "Indo-Malaysian")))

# 1. Correlation analysis
cat("\n[1] Correlation matrix:\n")
cor_matrix_inter1 <- cor(context1_inter[, c("dist_climate_mean", "dist_climate_var", "dist_topo", "dist_landmass")])
print(round(cor_matrix_inter1, 3))
# Correlation display
cor_melted <- melt(cor_matrix_inter1)
ggplot(cor_melted, aes(x = Var1, y = Var2, fill = value)) +
...
# 2. OLS model
model1_inter <- glm(beta_sim ~ dist_climate_mean + dist_climate_var
                    + dist_topo + + dist_landmass,
                    data = context1_inter, family = gaussian)
# 3. Summary of the model
cat("\n[2] Summary of the model:\n")
print(summary(model1_inter))
...
# Context 2: Andean vs. Neotropical + Afrotropical
# ================================================================

# Intra A (Andean)
cat("\n--- Intra A (Andean) ---\n")
context2_intraA <- pairs_df %>% 
  filter(phyloregion_i_name == "Andean", 
         phyloregion_j_name == "Andean")
...
# Between A and B (Chile-Patagonian vs Neotropical + Afrotropical)
cat("\n\n--- Between A and B (Andean vs Neotropical + Afrotropical) ---\n")
context2_inter <- pairs_df %>% 
  filter((phyloregion_i_name == "Andean" & phyloregion_j_name %in% c("Neotropical", "Afrotropical")) |
           (phyloregion_j_name == "Andean" & phyloregion_i_name %in% c("Neotropical", "Afrotropical")))

# 1. Correlation analisys
cat("\n[1] Correlation matrix:\n")
cor_matrix_inter2 <- cor(context2_inter[, c("dist_climate_mean", "dist_climate_var", "dist_topo", "dist_landmass")])
print(round(cor_matrix_inter2, 3))
...
# 2. OLS model
model2_inter <- glm(beta_sim ~ dist_climate_mean + dist_climate_var
                    + dist_topo + dist_landmass,
                    data = context2_inter, family = gaussian)
# 3. Summary of the model
cat("\n[2] Summary of the model:\n")
...
# 1. Correlation analisys
cat("\n[1] Correlation matrix:\n")
cor_matrix_inter3 <- cor(context3_inter[, c("dist_climate_mean", "dist_climate_var", "dist_topo","dist_landmass")])
print(round(cor_matrix_inter3, 3))
...
# 2. OLS model
model3_inter <- glm(beta_sim ~ dist_climate_mean + dist_climate_var
                    + dist_topo + dist_landmass,
                    data = context3_inter, family = gaussian)
...
# 1. Correlation analisys
cat("\n[1] Correlation matrix:\n")
cor_matrix_inter4 <- cor(context4_inter[, c("dist_climate_mean", "dist_climate_var", "dist_topo","dist_landmass")])
print(round(cor_matrix_inter4, 3))
# Correlation display
cor_melted <- melt(cor_matrix_inter4)
ggplot(cor_melted, aes(x = Var1, y = Var2, fill = value)) +
...
# 2. OLS model
model4_inter <- glm(beta_sim ~ dist_climate_mean + dist_climate_var
                    + dist_topo + dist_landmass,
                    data = context4_inter, family = gaussian)
# 3. Summary of the model
cat("\n[2] Summary of the model:\n")
...
model_list <- list(
  ...
  "The remaining groups" = model1_intraB,
  "Australian vs. The remaining groups" = model1_inter,
  "Andean" = model2_intraA,
  "Neotropical - Afrotropical" = model2_intraB,
  "Andean vs. Neotropical - Afrotropical" = model2_inter,
  "Neotropical" = model3_intraA,
  "Afrotropical" = model3_intraB,
  "Neotropical vs. Afrotropical " = model3_inter,
  "Holartic" = model4_intraA,
...
# Add context and readable variable names
  tidy_df$Context <- context_name
  ...
  tidy_df$Variable <- dplyr::case_when(
    ...
    tidy_df$term == "dist_climate_var"  ~ "Climate variability",
    tidy_df$term == "dist_topo"         ~ "Topographical heterogeneity",
    tidy_df$term == "dist_landmass"     ~ "Landmass discontinuity",
    tidy_df$term == "(Intercept)"       ~ "Intercept",
    TRUE ~ tidy_df$term)
...
relimp_results <- relimp_results %>%
  mutate(Variable = dplyr::case_when(
    ...
    Variable == "dist_climate_var"  ~ "Climate variability",
    Variable == "dist_topo"         ~ "Topographical heterogeneity",
    Variable == "dist_landmass"     ~ "Landmass discontinuity",
    TRUE ~ Variable))
...
# SAVE
setdiff(tabla_resultados$Variable, relimp_results$Variable)
write.csv(tabla_combinada, "results/table/results_OLS_models.csv", row.names = FALSE)
...
#Create a context metadata table (explicit)
context_metadata <- tibble(
  Context = c(
    "Australian",
    ...
    "Andean vs. Neotropical - Afrotropical",
    "Neotropical",
    "Afrotropical",
    "Neotropical vs. Afrotropical ",
    "Holartic",
    "Indo-Malaysian",
    "Holartic vs. Indo-Malaysian"),
  Biogeo_Group = c(
    "Australian vs. The remaining groups",
    "Australian vs. The remaining groups",
    "Australian vs. The remaining groups",
...
plot_data <- tabla_combinada %>%
  filter(Variable %in% c(
    ...
    "Landmass discontinuity")) %>%
  left_join(context_metadata, by = "Context") %>%
  mutate(
    Biogeo_Group = factor(
      Biogeo_Group,
      levels = c(
    ...
    Context_Type = factor(
      Context_Type,
      levels = c("IntraA", "IntraB", "Inter"),
      labels = c("Within A", "Within B", "Between groups")),
    Variable = factor(
      Variable,
      levels = c(
        "Average climate",
        ...
        "Landmass discontinuity")),
    R2_real = R2_percent) %>%
  group_by(Biogeo_Group, Context_Type) %>%
  mutate(
    Total_R2 = sum(R2_real, na.rm = TRUE),
    R2_percent = ifelse(Total_R2 > 0, (R2_real / Total_R2) * 100, 0)) %>%
  ungroup()
...
final_plot <- ggplot(plot_data,
                     aes(x = Context_Type, y = R2_percent, fill = Variable)) +
  geom_col(width = 0.7) +
  geom_text(
    aes(label = ifelse(R2_real > 0.5, round(R2_real, 1), "")),
  ...
  scale_fill_manual(
    values = c(
      "Average climate" = "#4C72B0",
      "Climate variability" = "#55A868",
      "Topographical heterogeneity" = "#C44E52",
      "Landmass discontinuity" = "#8172B2"),
    name = "") +
  labs(x = NULL, y = expression(R^2~"(%)")) +
...
# Save
ggsave("results/Figures/Drivers1.png", final_plot, 
       width = 25, height =18 , units = "cm", dpi = 400, bg = "white")
##################################
final_plot1 <- ggplot(plot_data,
                     aes(x = Context_Type, y = R2_percent, fill = Variable)) +
  geom_col(width = 0.7) +
  facet_wrap(~ Biogeo_Group, ncol = 2) +
  scale_fill_manual(
...
final_plot1
# Save
ggsave("results/Figures/Drivers1.png", final_plot1, 
       width = 25, height =18 , units = "cm", dpi = 400, bg = "white")
